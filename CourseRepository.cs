...
namespace CourseApp.Repository
{
    public class CourseRepository
    {
        ...
        public int GetCourseTimeLimit(int courseId)
        {
            ...
            {
                connection.Open();
                string query = "SELECT TimeLimit FROM Courses WHERE CourseId = @courseId";
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@courseId", courseId);
                    var result = command.ExecuteScalar();
        ...
        // Add method to get timed reward amount
        public int GetTimedRewardAmount(int courseId)
        {
            int amount = 0;
            using (SqlConnection connection = DataLink.GetConnection())
            {
                connection.Open();
                string query = "SELECT TimedRewardAmount FROM Courses WHERE CourseId = @courseId";
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    command.Parameters.AddWithValue("@courseId", courseId);
                    var result = command.ExecuteScalar();
